...
// --- Correct NBA Date Function (no timezone needed) ---
const getNBAFormattedDate = (offsetDays = 0) => {
  const now = new Date();
  now.setDate(now.getDate() + offsetDays);

  const formattedDate = `${now.getFullYear()}${(now.getMonth() + 1).toString().padStart(2, "0")}${now.getDate().toString().padStart(2, "0")}`;
  const readableDate = now.toLocaleDateString("en-US", { month: "long", day: "numeric", year: "numeric" });
...
app.post("/api/smart", async (req, res) => {
  ...
  try {
    ...
    if (isNBAScheduleRequest(prompt)) {
      const offset = isYesterdayNBARequest(prompt) ? -1 : 0;
      const { formattedDate, readableDate } = getNBAFormattedDate(offset);

      const games = await fetchNBASchedule(formattedDate);
