...
const fetchSportsData = async (teamKey, type) => {
  ...
  try {
    if (type === 'scores' || type === 'summary') {
      const url = `${baseUrl}/${sportId}/events?date=${date}`;
      ...
      if (!teamGame) return `No game found today for ${teamKey}.`;
      ...
      const url = `${baseUrl}/${sportId}/schedule`;
      const res = await axios.get(url, { headers });
      const games = res.data.events || [];
      const teamGame = games.find((game) => {
        return (
      ...
      if (!teamGame) return `No upcoming games found for ${teamKey}.`;
      return `${teamGame.teams.away} vs ${teamGame.teams.home} at ${teamGame.event_date}`;
    } else if (type === 'odds') {
      const url = `${baseUrl}/${sportId}/odds`; // default returns current odds
      const res = await axios.get(url, { headers });
