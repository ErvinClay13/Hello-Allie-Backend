...
app.post("/api/smart", async (req, res) => {
  ...
  try {
    if (/what('| i)?s the time|current time|local time/i.test(prompt)) {
...
app.post("/api/schedule", async (req, res) => {
  ...
  try {
    ...
    // === QUERY EVENTS ===
    if (/(what('| i)?s on my schedule|show my schedule|what('| i)?s my schedule|show my events)/i.test(lowerPrompt)) {
      const snapshot = await db.collection("schedules").orderBy("createdAt", "desc").get();
      const events = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
      return res.status(200).json({ events });
    }

    // === DELETE EVENT BY TASK ===
    if (/delete|remove/i.test(lowerPrompt)) {
      const deleteMatch = lowerPrompt.match(/delete|remove (.+?) from/i);
      const keyword = deleteMatch ? deleteMatch[1].trim() : null;
      if (keyword) {
        const snapshot = await db.collection("schedules").where("task", "==", keyword).get();
        if (snapshot.empty) {
          return res.status(404).json({ message: `No event found with task "${keyword}".` });
        }
        const batch = db.batch();
        snapshot.forEach(doc => batch.delete(doc.ref));
        await batch.commit();
        return res.status(200).json({ message: `Deleted all events matching "${keyword}".` });
      } else {
        return res.status(400).json({ message: "Could not parse delete keyword." });
      }
    }

    // === ADD NEW EVENT ===
    const taskMatch = prompt.match(/remind me to (.+?) at/i) || prompt.match(/schedule (.+?) at/i) || prompt.match(/add to calendar[:]? (.+?) at/i);
    ...
    if (taskMatch && timeMatch && dateMatch) {
      ...
      let time = timeMatch[1].trim().toLowerCase();
      const date = dateMatch[1].trim();

      if (!time.includes("am") && !time.includes("pm")) {
        time += " am"; // default if missing
      }
      ...
      return res.status(200).json({ message: `Saved "${task}" at ${time} on ${date}.`, id: docRef.id });
    }

    return res.status(200).json({ message: "Could not parse date or time." });
