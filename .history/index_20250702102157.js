// ✅ FINAL FULL index.js with ALL UTILITY FUNCTIONS
...
app.post("/api/smart", async (req, res) => {
  ...
  try {
    ...
    if (isNameQuery(prompt)) {
      return res.json({ result: "My name is Allie, short for Artificial Language Learning & Interaction Engine." });
    }
    ...
    const wrapper = personalityPrompts[personalityKey];
    const messages = [
...
app.post("/api/schedule", async (req, res) => {
  ...
  try {
    if (/what('| i)?s on my schedule|show schedule/i.test(prompt)) {
    ...
    if (/delete|remove/i.test(prompt)) {
      const deleteMatch = prompt.match(/delete (.+)/i) || prompt.match(/remove (.+)/i);
      if (deleteMatch && deleteMatch[1]) {
        const taskToDelete = deleteMatch[1].trim().toLowerCase();
        const snapshot = await db.collection("schedules").where("task_lower", "==", taskToDelete).get();
        if (snapshot.empty) {
          return res.json({ message: `No event found with task "${taskToDelete}".` });
        }
        const batch = db.batch();
        snapshot.forEach(doc => batch.delete(doc.ref));
        await batch.commit();
        return res.json({ message: `Deleted event "${taskToDelete}".` });
      } else {
        return res.json({ message: "Please specify the exact event to delete." });
      }
    }
