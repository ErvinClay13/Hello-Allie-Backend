...
app.post("/api/smart", async (req, res) => {
  ...
  try {
    ...
    if (/what('| i)?s your name|who are you|tell me about yourself/i.test(prompt)) {
    ...
    if (/(tell me a joke|dad joke|make me laugh|joke|say something funny)/i.test(prompt)) {
...
app.post("/api/schedule", async (req, res) => {
  ...
  try {
    ...
    // === CREATE EVENT ===
    const taskMatch = lowerPrompt.match(/remind me to (.+?) at/i);
    const timeMatch = lowerPrompt.match(/at ([0-9]{1,2}(?::[0-9]{2})?\s?(am|pm)?)/i);
    const dateMatch = lowerPrompt.match(/(today|tomorrow|monday|tuesday|wednesday|thursday|friday|saturday|sunday)/i);
    ...
    if (lowerPrompt.includes("delete")) {
      const taskToDelete = lowerPrompt.replace("delete", "").trim();
      ...
      snapshot.forEach((doc) => {
        const docTask = doc.data().task.toLowerCase().trim();
        if (docTask.includes(taskToDelete)) {
      ...
      if (deletedCount === 0) {
        return res.status(200).json({ message: `No events found containing "${taskToDelete}".` });
      }

      await batch.commit();
      return res.status(200).json({ message: `Deleted ${deletedCount} event(s) containing "${taskToDelete}".` });
    }

    // === LIST EVENTS ===
    if (lowerPrompt.includes("what") && lowerPrompt.includes("schedule")) {
