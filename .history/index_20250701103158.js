...
app.post("/api/schedule", async (req, res) => {
  ...
  try {
    const addMatch = prompt.match(/(?:remind me to|schedule|add)\s(.+?)(?:\s(?:at|for)\s([0-9]{1,2}(?::[0-9]{2})?\s?(?:am|pm)?))?(?:\s(?:on|for)?\s?(today|tomorrow|monday|tuesday|wednesday|thursday|friday|saturday|sunday))?/i);
    ...
    if (addMatch) {
      ...
      const time = addMatch[2] ? addMatch[2].trim() : "unspecified";
      const date = addMatch[3] ? addMatch[3].trim() : "unspecified";

      const docRef = await db.collection("schedules").add({
        task,
        time: time.toLowerCase().includes("am") || time.toLowerCase().includes("pm") ? time : time + " (unspecified am/pm)",
        date,
        createdAt: new Date(),
      });

      return res.status(200).json({
        message: `Added "${task}" at ${time} on ${date}.`,
        id: docRef.id,
      });
    }

    else if (deleteMatch) {
      const taskToDelete = deleteMatch[1].trim().toLowerCase();
      const snapshot = await db.collection("schedules").get();
      const matchingDocs = snapshot.docs.filter(doc =>
        doc.data().task.toLowerCase().includes(taskToDelete)
      );

      if (matchingDocs.length === 0) {
        return res.status(404).json({ message: `No event found matching "${taskToDelete}".` });
      }

      await Promise.all(matchingDocs.map(doc => doc.ref.delete()));
      return res.status(200).json({ message: `Deleted ${matchingDocs.length} event(s) containing "${taskToDelete}".` });
    }
