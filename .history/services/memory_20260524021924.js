...
async function getCommunicationStyleContext(uid) {
  try {
    ...
    var data = snap.data();
    if (!data) return "";
    var style = data.communicationStyle;
    if (!style) return "";
    var parts = [];
...
async function getMoodContext(uid) {
  try {
    // Use simple get without orderBy to avoid requiring a composite index
    var snap = await db.collection("users").doc(uid).collection("moods").limit(7).get();
    ...
    var recent = entries.slice(0, 3).map(function(e) { return e.mood; }).filter(Boolean).join(", ");
    var trend = avg >= 7 ? "positive" : avg <= 3 ? "low" : "stable";
    return recent ? "User mood trend: " + trend + " (avg " + avg.toFixed(1) + "/10). Recent moods: " + recent + "." : "";
  } catch (err) {
    console.warn("getMoodContext error (non-critical):", err && err.message);
    return "";
  }
}
...
async function getRecentSummaries(uid, limit) {
  ...
  try {
    ...
    var snap = await db.collection("chats")
      ...
      .get();
    return snap.docs
      .map(function(d) { return d.data().summary; })
      .filter(Boolean)
      .join(" | ");
  } catch (err) {
    console.warn("getRecentSummaries error (non-critical):", err && err.message);
    return "";
  }
}
