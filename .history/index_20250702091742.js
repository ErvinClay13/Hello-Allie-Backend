...
const fetchWeather = async (city) => {
  try {
    ...
    return `The current weather in ${data.name}, ${data.sys.country} is ${data.weather[0].description} with ${tempF.toFixed(1)}°F, humidity ${humidity}% and wind ${windSpeedMph.toFixed(1)} mph.`;
  } catch (error) {
...
app.post("/api/schedule", async (req, res) => {
  ...
  try {
    // View schedule
    if (/what('| i)?s on my schedule|what('| i)?s my schedule|show schedule/i.test(prompt)) {
    ...
    if (/delete|remove/i.test(prompt)) {
      ...
      if (deleteMatch && deleteMatch[1]) {
        ...
        if (snapshot.empty) return res.json({ message: `No event found with task "${taskToDelete}".` });
