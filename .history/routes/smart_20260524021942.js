...
const {
  ...
  fetchNBAScores, fetchNFLScores, fetchMLBScores,
  fetchNHLScores, fetchNCAAFBScores, fetchMLSScores,
...
const {
  ...
  detectMemoryCommand, buildMemoryContext, getMoodContext,
  analyzeCommunicationStyle, getCommunicationStyleContext,
...
router.post("/", requireAuth, checkUsageLimit, async (req, res) => {
  try {
    ...
    else if (intents.nba(userText))    realWorldContext = await fetchNBAScores(offset);
    else if (intents.nfl(userText))    realWorldContext = await fetchNFLScores(offset);
    else if (intents.mlb(userText))    realWorldContext = await fetchMLBScores(offset);
    else if (intents.nhl(userText))    realWorldContext = await fetchNHLScores(offset);
    else if (intents.ncaafb(userText)) realWorldContext = await fetchNCAAFBScores(offset);
    else if (intents.mls(userText))    realWorldContext = await fetchMLSScores(offset);
    else if (intents.sports(userText)) {
      const [nba, nfl] = await Promise.all([fetchNBAScores(offset), fetchNFLScores(offset)]);
      const parts = [nba, nfl].filter(Boolean);
      realWorldContext = parts.join("\n\n");
    }
    else if (intents.news(userText))   realWorldContext = await fetchNews();
    ...
    const [userProfile, memories, recentSummaries, moodCtx, styleCtx] = await Promise.all([
      getUserProfile(uid),
      getMemories(uid),
      getRecentSummaries(uid, 3),
      getMoodContext(uid),
      getCommunicationStyleContext(uid),
    ]);
    ...
    const fullContext       = [memoryContext, moodCtx, styleCtx].filter(Boolean).join(" ");
    ...
    const system = buildSystemPrompt({
      ...
      language:         lang,
      realWorldContext: [realWorldContext, fullContext].filter(Boolean).join("\n\n") || null,
    });
    ...
    if (historyDocs.length > 0 && historyDocs.length % 10 === 0) {
      ...
      // Run summarization and style analysis in parallel, non-blocking
      Promise.all([
        summarizeConversation(allMsgs).then((s) => saveConversationSummary(chatId, s)),
        analyzeCommunicationStyle(uid, allMsgs),
      ]).catch(() => {});
    }
    ...
    return res.status(500).json({ error: "Smart AI failed", detail: err?.message });
  }
});
