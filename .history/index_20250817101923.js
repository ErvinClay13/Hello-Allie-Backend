...
const fetchNBAScoreboard = async (language = "en") => {
  try {
...
app.post("/api/smart", async (req, res) => {
  const { prompt, personality, language } = req.body;

  const personalityPrompts = {
    friendly: "Respond in a friendly, helpful way.",
    sassy: "Respond with a sassy and witty attitude.",
    motivational: "Respond in an encouraging, uplifting tone.",
    humorous: "Respond with humor and a lighthearted tone.",
  };

  const systemPrompt = `${personalityPrompts[personality] || personalityPrompts.friendly}${language === "es" ? " Respond in Spanish." : ""}`;
...
app.get("/api/joke", async (req, res) => {
  ...
  try {
    ...
    const result = lang === "es" ? `Aquí tienes un chiste: ${joke}` : joke;
    res.json({ result });
  } catch (error) {
...
app.get("/api/nba/scores", async (req, res) => {
  const lang = req.query.language || "en";
  const result = await fetchNBAScoreboard(lang);
  res.json({ result });
});
...
app.post("/api/schedule/parse", (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ error: "Schedule parse failed." });
  }
});

app.get("/api/worldtime", async (req, res) => {
  const lang = req.query.language || "en";
  const result = await fetchLocalTimeByIP(lang);
  res.json({ result });
});
