...
// Helper: Detect weather request and extract city
...
// Fetch weather using RapidAPI with imperial units (Fahrenheit)
const fetchWeather = async (city) => {
  try {
    const url = `https://open-weather13.p.rapidapi.com/city/${city}/US?units=imperial`;
    ...
    return `The current weather in ${data.name}, ${data.sys.country} is ${data.weather[0].description} with a temperature of ${tempF.toFixed(
      1
    )}°F, humidity of ${humidity}% and wind speed of ${windSpeed} m/s.`;
  } catch (error) {
...
// Smart AI (handles weather or general prompts)
