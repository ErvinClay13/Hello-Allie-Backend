...
app.get("/api/nba/scores", async (req, res) => {
  try {
    const response = await axios.get("https://therundown-therundown-v1.p.rapidapi.com/sports/basketball/nba/events", {
      headers: {
        "X-RapidAPI-Key": process.env.RAPIDAPI_KEY,
    ...
    res.status(500).json({ error: "NBA scores fetch failed." });
  }
});
