...
// --- Fetch NBA Live Scoreboard with (LIVE 🔴) and Quarter/Clock
  ...
  try {
    ...
    const sortedEvents = events.sort((a, b) => {
      const getPriority = (status) => {
        if (status === "STATUS_IN_PROGRESS") return 0;
        if (status === "STATUS_FINAL") return 1;
        return 2;
      };
      return getPriority(a.status?.type?.name) - getPriority(b.status?.type?.name);
    });

    const gameSummaries = sortedEvents.map((event) => {
      ...
      if (gameStatus === "STATUS_IN_PROGRESS") {
        statusLabel = `(LIVE 🔴 Q${period} ${clock})`;
      ...
      return `${awayName} ${awayScore} - ${homeScore} ${homeName} ${statusLabel}`;
    }).join("\n");
