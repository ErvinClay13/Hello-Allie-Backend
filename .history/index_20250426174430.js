...
const fs = require("fs");
const axios = require("axios"); // Needed for jokes & NBA APIs
...
const isNBAScheduleRequest = (text) =>
  /(nba schedule|nba games|nba today)/i.test(text);
...
const fetchNBASchedule = async (date) => {
  try {
    ...
    console.log("🏀 NBA API Raw Response:", response.data); // Useful for debugging

    // Safely check for 'games' array inside response.data
    const gamesList = Array.isArray(response.data.games) ? response.data.games : [];

    if (gamesList.length === 0) {
      return `No NBA games found for that date.`;
    }

    const gameSummaries = gamesList.map((game) => {
      return `${game.home_team} vs ${game.away_team} at ${game.time}`;
    }).join("\n");
