...
app.get("/api/usage", async (req, res) => {
  ...
  requireAuth(req, res, async () => {
    const { getUsageCount, FREE_LIMIT } = require("./middleware/usageLimit");
    const { admin } = require("./config/firebaseAdmin");
    try {
      const userSnap = await require("./config/firebaseAdmin").db
        .collection("users").doc(req.uid).get();
      const plan  = (userSnap.data() && userSnap.data().plan) || "free";
      const count = plan === "pro" ? 0 : await getUsageCount(req.uid);
      const limit = plan === "pro" ? -1 : FREE_LIMIT;
      res.json({ count, limit, plan, remaining: plan === "pro" ? -1 : Math.max(0, limit - count) });
    } catch (err) {
