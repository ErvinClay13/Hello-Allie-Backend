...
const SYSTEM_PROMPT =
  ...
  "You are Allie, a concise, friendly AI assistant. Be helpful and on-topic.";
...
const personalityPrompts = {
  friendly: "You speak in a warm, encouraging, friendly tone.",
  sassy: "You are witty and playfully sarcastic, but never rude or hurtful.",
  motivational: "You are an energetic motivator who inspires users to take action.",
  humorous: "You are funny and clever; keep it light and tasteful.",
};




// === HELPERS FOR SMART ROUTE ===

function buildSystemPrompt({ language, personalityKey }) {
  const tone = personalityPrompts[personalityKey] || personalityPrompts.friendly;
  const langHint =
    language === "es" ? "Respond in Spanish." : "Respond in clear, natural English.";
  return `${SYSTEM_PROMPT} ${tone} ${langHint}`;
}

function mapHistoryToModelMessages(historyDocs) {
  // docs ascending by timestamp
  return historyDocs.map((m) => {
    const role =
...
async function fetchChatHistoryForContext(chatId) {
  const snap = await db
    .collection("chats")
    .doc(chatId)
    .collection("messages")
...
async function writeAssistantReply(chatId, text) {
...
async function generateReplyWithFallback({ system, messages }) {
  try {
    ...
    // Retry with a shorter context
    const shortMsgs = messages.slice(-8);
    const resp = await openai.chat.completions.create({
      model: MODEL,
      messages: [{ role: "system", content: system }, ...shortMsgs],
      temperature: 0.7,
      max_tokens: 350,
    });
    return resp.choices?.[0]?.message?.content?.trim() || "…";
...
app.post("/api/smart", async (req, res) => {
  try {
    ...
    // New (from app): { chatId, message, language, personality }
    // Old: { prompt, conversationHistory, language, mode/personality }
    const {
      chatId,
      message,
      language: langNew,
    ...
    const language = (langNew || langOld || "en").toLowerCase();
    const personalityKey = ((perNew || perOld || modeOld || "friendly") + "").toLowerCase();

    // Quick-intent shortcuts
    if (
      /what('| i)?s the time|what('| i)?s the date|current time|current date|local time/i.test(userText)
    ) {
    ...
    if (isNameQuery(userText)) {
      return res.json({
        result:
          language === "es"
            ? "Me llamo Allie, abreviatura de Artificial Language Learning & Interaction Engine. ¡Estoy aquí para ayudarte en lo que necesites!"
            : "My name is Allie, short for Artificial Language Learning & Interaction Engine. I’m here to help you with whatever you need!",
      });
    }
    if (isDadJokeRequest(userText)) {
      return res.json({ result: await fetchDadJoke() });
    }
    if (isNBAScheduleRequest(userText)) {
      const offset = detectNBADateOffset(userText);
    ...
    // Require chatId to write reply into Firestore
    if (!chatId) {
      return res.status(400).json({ error: "chatId is required" });
    }

    const system = buildSystemPrompt({ language, personalityKey });

    // Context = Firestore history (plus old flow history if provided), then current user message
    const fsHistoryDocs = await fetchChatHistoryForContext(chatId);
    const fsMsgs = mapHistoryToModelMessages(fsHistoryDocs);
    const mergedHistory = [
      ...fsMsgs,
    ...
    // Generate reply
    const reply = await generateReplyWithFallback({ system, messages: mergedHistory });

    // Persist assistant reply & update parent chat
    await writeAssistantReply(chatId, reply);
...
app.post("/api/schedule", async (req, res) => {
  ...
  try {
    ...
    let dateMatch = prompt.match(
      /on (today|tomorrow|monday|tuesday|wednesday|thursday|friday|saturday|sunday)/i
    );

    if (!dateMatch) {
      dateMatch = prompt.match(
...
app.post("/api/schedule/delete", async (req, res) => {
  ...
  try {
    ...
    if (matches.length === 1) {
      await db.collection("schedules").doc(matches[0].id).delete();
...
// app.post("/api/smart", async (req, res) => {
//   const { prompt, conversationHistory = [], personality, mode, language = "en" } = req.body;
//   const personalityKey = (personality || mode || "friendly").toLowerCase();

//   try {
//     if (
//       /what('| i)?s the time|what('| i)?s the date|current time|current date|local time/i.test(prompt)
//     ) {
//       return res.json({ result: await fetchLocalTimeByIP() });
//     }
...
//     // Translate wrapper if needed
//     const translatedWrapper =
//       language === "es"
//         ? `Responde en español. ${wrapper}`
//         : wrapper;

//     const messages = [
//       { role: "system", content: translatedWrapper },
...
//     if (matches.length === 1) {
//       console.log("Deleting single match ID:", matches[0].id);
//       await db.collection("schedules").doc(matches[0].id).delete();
//       return res.json({ message: `Deleted "${matches[0].task}".` });
//     }
...
// ✅ FINAL FULL VERSION: index.js — Complete backend with full language + personality + NBA + weather + world time + transcription + smart scheduling support
...
// const fetchLocalTimeByIP = async (language = "en") => {
//   try {
//     const response = await axios.get(`https://api.ipgeolocation.io/timezone?apiKey=${process.env.IPGEOLOCATION_API_KEY}`);
//     const { date_time_txt, timezone, geo } = response.data;
//     return language === "es"
//       ? `La hora local actual es ${date_time_txt} en ${geo.city}, ${geo.country_name} (${timezone}).`
//       : `The current local time is ${date_time_txt} in ${geo.city}, ${geo.country_name} (${timezone}).`;
//   } catch (error) {
//     console.error("❌ World Time API error:", error);
//     return language === "es" ? "Lo siento, no pude obtener la hora local actual." : "Sorry, I couldn't fetch the current local time.";
//   }
// };
...
// const fetchNBAScoreboard = async (date, language = "en") => {
//   try {
...
//     });
//     const events = response.data?.response?.Events || [];
//     if (events.length === 0) return language === "es" ? "No se encontraron juegos de la NBA para esa fecha." : `No NBA games found for that date.`;
...
//       let gameLine = `${awayName} ${awayScore} - ${homeScore} ${homeName}`;
//       if (gameStatus === "STATUS_IN_PROGRESS") {
//         gameLine += language === "es" ? ` (EN VIVO 🔴 Q${period} ${clock})` : ` (LIVE 🔴 Q${period} ${clock})`;
//         liveGames.push(gameLine);
//       } else if (gameStatus === "STATUS_FINAL") {
//         gameLine += language === "es" ? ` (Finalizado)` : ` (Final)`;
//         finalGames.push(gameLine);
//       } else {
//         gameLine += language === "es" ? ` (Programado)` : ` (Scheduled)`;
//         scheduledGames.push(gameLine);
//       }
//     }

//     let output = "";
//     if (liveGames.length) output += `${language === "es" ? "🔴 EN VIVO AHORA" : "🔴 LIVE NOW"}:
// ${liveGames.join("\n")}

// `;
//     if (finalGames.length) output += `${language === "es" ? "🏁 RESULTADOS FINALES" : "🏁 FINAL SCORES"}:
// ${finalGames.join("\n")}

// `;
//     if (scheduledGames.length) output += `${language === "es" ? "🕒 PRÓXIMOS JUEGOS" : "🕒 UPCOMING GAMES"}:
// ${scheduledGames.join("\n")}`;
//     return output.trim();
//   } catch (error) {
//     console.error("❌ NBA Scoreboard API error:", error);
//     return language === "es" ? "No se pudieron obtener los puntajes en vivo." : "Failed to fetch NBA live scores.";
//   }
// };

// const fetchWeather = async (city, language = "en") => {
//   try {
//     const formattedCity = city.replace(/,/g, "").split(" ")[0];
//     const response = await axios.get(`https://open-weather13.p.rapidapi.com/city/${formattedCity}/US?units=imperial`, {
//       headers: {
//         "X-RapidAPI-Key": process.env.RAPIDAPI_KEY,
//         "X-RapidAPI-Host": "open-weather13.p.rapidapi.com",
//       },
...
//     const windSpeedMph = data.wind.speed;
//     return language === "es"
//       ? `El clima actual en ${data.name}, ${data.sys.country} es ${data.weather[0].description} con una temperatura de ${tempF.toFixed(1)}°F, humedad del ${humidity}% y velocidad del viento de ${windSpeedMph.toFixed(1)} mph.`
//       : `The current weather in ${data.name}, ${data.sys.country} is ${data.weather[0].description} with a temperature of ${tempF.toFixed(1)}°F, humidity of ${humidity}% and wind speed of ${windSpeedMph.toFixed(1)} mph.`;
//   } catch (error) {
//     console.error("❌ Weather API error:", error);
//     return language === "es" ? `No pude obtener el clima para "${city}".` : `Sorry, I couldn't retrieve the weather for "${city}".`;
//   }
// };

// const fetchDadJoke = async (language = "en") => {
//   try {
...
//     return language === "es" ? `Aquí tienes un chiste: ${joke}` : joke;
//   } catch (error) {
//     console.error("❌ Dad Joke API error:", error);
//     return language === "es" ? "No pude obtener un chiste en este momento." : "Failed to fetch a dad joke.";
//   }
// };
